...
**Correlation Edges.** The third edge type is dynamic. A sixty-day rolling Pearson correlation is computed between the daily returns of every pair of stocks. If the correlation between two stocks exceeds a threshold of 0.6, an undirected edge is created between them for that time step. If the correlation drops below 0.6, the edge is removed. This captures temporary co-movement clusters, such as the IT sector rallying together when the rupee weakens, or metals stocks moving together when global commodity prices shift. The graph has 147 correlation edges in the snapshot taken from the dashboard at the time of evaluation.
...
A maximum position size of twelve percent per stock prevents excessive concentration in any single company. If the softmax output allocates more than twelve percent to any stock, the excess is redistributed proportionally to the other stocks.
...
All responses follow a consistent JSON schema defined by Pydantic models, which makes the React frontend code predictable and allows the Swagger documentation to accurately describe every endpoint's response structure.

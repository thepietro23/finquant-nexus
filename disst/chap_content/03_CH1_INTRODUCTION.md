...
The National Stock Exchange of India is among the highest-volume equity exchanges in Asia, and its benchmark, the NIFTY 50, tracks fifty large-cap companies spread across thirteen sectors of the Indian economy. [2] These companies together account for more than sixty percent of total NSE trading value by market capitalisation. Any investor with Indian equity exposure uses this index as the reference point, whether directly or just as a comparison.

Managing a NIFTY 50 portfolio is not simply a matter of choosing which stocks to hold. The harder question is how much capital goes into each stock at any given point, and how those weights should shift as conditions change. This is the portfolio allocation problem. It is considerably harder than stock selection alone, and getting it right consistently across bull runs, sideways stretches, and sudden corrections is something even experienced fund managers struggle with.

The theoretical starting point for this problem comes from Markowitz's 1952 paper. [1] He showed that if expected returns and the covariance of a set of assets are known, mathematics can find the allocation that minimises risk for any given return level. The set of all such solutions forms the efficient frontier, which became the central idea in modern portfolio theory. The framework is still taught in every finance course today.

But the assumptions underneath it break down badly in real markets. Stock returns are assumed to follow a normal distribution. Indian equities do not behave that way. Sharp single-day drops and fast recoveries occur far more often than any normal distribution predicts, especially around election results, RBI rate decisions, or sector-level regulatory announcements. [4] Also, correlations between stocks are treated as fixed over time. They are not. During a sell-off, stocks that had been moving independently suddenly move together. That is precisely when diversification is supposed to protect the portfolio, and it is exactly when the model fails.

A second problem compounds this. For fifty stocks, computing a full covariance matrix means estimating 1225 pairwise correlations. Small estimation errors, which are unavoidable when using historical data, can shift the computed optimal weights dramatically. The method is too sensitive to input noise to be reliable outside controlled settings.

So research has moved toward learning-based methods. Reinforcement learning in particular has been studied for portfolio management. [9] An RL agent does not need a pre-specified return model. It interacts with historical data, makes allocation decisions, observes what happens to the portfolio, and updates its policy over time. This process, run over financial time series, can find allocation strategies that adapt to changing conditions rather than assuming stability. Also, the environment can be designed to directly enforce constraints like position limits and drawdown controls, which is difficult to handle cleanly in classical optimisation.
...
This work covers the design, implementation, and evaluation of FINQUANT-NEXUS as a locally deployed simulation system. The stock universe is forty-four NIFTY 50 constituent stocks for which a complete price history was available through the Yahoo Finance API. Six index constituents were excluded because of incomplete data. The dataset spans January 2015 to December 2025, divided into training (2015 to 2021), validation (2022 to 2023), and test (2024 to 2025) periods.
...
Two components exist in the broader codebase but are outside the scope of this dissertation: a Quantum Approximate Optimisation Algorithm (QAOA) module and a Neural Architecture Search (NAS/DARTS) module. Both require separate theoretical treatment and are not included in the eight-tab dashboard interface this dissertation covers.
